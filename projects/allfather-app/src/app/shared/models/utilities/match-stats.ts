...
export function complimentaryLegendsWeights(
    ...
    console.debug(`Complimentary Legends for "${legendId}":`, avgCompLegendWeights);
...
export function matchAvgStatWeights(matchStats: Partial<AvgMatchStats>, statBounds: StatBounds, statWeights: StatWeights): AvgStatWeights {
    ...
    const totalWeight = Math.min(avgWinWeight + avgPlacementWeight + avgDamageWeight + avgEliminationWeight + avgDurationWeight, 1);
    return { avgWinWeight, avgPlacementWeight, avgDamageWeight, avgEliminationWeight, avgDurationWeight, totalAvgWeight: totalWeight };
}
