...
describe("Rank", () => {
    ...
    describe("computed properties", () => {
        ...
        it("should compute tierDivisionProgress", () => {
            // Arrange / Act
            const bronzeRank4 = new Rank({ score: 1 });
            const masterRank = new Rank({ score: 10001 });

            // Assert
            expect(bronzeRank4.score).withContext(`bronzeRank4`).toBe(1);
            expect(masterRank.score).withContext(`masterRank`).toBe(10001);
        });

        it("should compute tierName", () => {
            // Arrange / Act
            const bronzeRank4ByScore = new Rank({ score: 1 });
            const bronzeRank4ByName = new Rank({ tierName: RankTierName.Bronze });
            const bronzeRank4ByBoth = new Rank({ score: 1, tierName: RankTierName.Master, tierDivision: 1 });
            const masterRankByScore = new Rank({ score: 10001 });
            const masterRankByName = new Rank({ tierName: RankTierName.Master });

            // Assert
            expect(bronzeRank4ByScore.tierName).withContext(`bronzeRankByScore4`).toBe("Bronze");
            expect(bronzeRank4ByName.tierName).withContext(`bronzeRankByName4`).toBe("Bronze");
            expect(bronzeRank4ByBoth.tierName).withContext(`bronzeRankByBoth4`).toBe("Bronze");
        ...
        it("should compute tierDivision", () => {
            // Arrange / Act
            const bronzeRank4ByScore = new Rank({ score: 1 });
            const bronzeRank4ByName = new Rank({ tierName: RankTierName.Bronze, tierDivision: 4 });
            const bronzeRank4ByBoth = new Rank({ score: 1, tierName: RankTierName.Bronze, tierDivision: 1 });
            const masterRankByScore = new Rank({ score: 10001 });
            const masterRankByBoth = new Rank({ score: 10001, tierName: RankTierName.Master, tierDivision: 1 });

            // Assert
            expect(bronzeRank4ByScore.tierDivision).withContext(`bronzeRankByScore4`).toBe("IV");
            expect(bronzeRank4ByName.tierDivision).withContext(`bronzeRankByName4`).toBe("IV");
            expect(bronzeRank4ByBoth.tierDivision).withContext(`bronzeRank4ByBoth`).toBe("IV");
        ...
        it("should compute tierDivisionMinScore", () => {
            ...
            const masterRank = new Rank({ score: 10001, tierName: RankTierName.Master, tierDivision: 1 });
    ...
    describe("by given Score and NOT name or division", () => {
        it("should map to Bronze by score", () => {
            // Arrange / Act
            const bronzeRank4 = new Rank({ score: 0, tierName: "Master", tierDivision: RankTierDivisionMap[1] });
            const bronzeRank3 = new Rank({ score: 300, tierName: "Master", tierDivision: RankTierDivisionMap[1] });
            const bronzeRank2 = new Rank({ score: 600, tierName: "Master", tierDivision: RankTierDivisionMap[1] });
            const bronzeRank1 = new Rank({ score: 900, tierName: "Master", tierDivision: RankTierDivisionMap[1] });
