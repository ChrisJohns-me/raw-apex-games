...
class Solver {
    ...
    constructor(public target: { r: number; g: number; b: number; hsl: () => any }) {
        this.targetHSL = target.hsl();
        this.reusedColor = new Color(0, 0, 0);
    }

    public solve(): { values: number[]; loss: number; filter: string } {
    ...
    public solveWide(): { values: number[]; loss: number } {
        const A = 5;
        const c = 15;
    ...
    public solveNarrow(wide: { values: number[]; loss: number }): { values: number[]; loss: number } {
        const A = wide.loss;
        const c = 2;
        const A1 = A + 1;
    ...
    public spsa(A: number, a: number[], c: number, values: number[], iters: number): { values: number[]; loss: number } {
        const alpha = 1;
    ...
    public css(filters: number[]): string {
        const fmt = (idx: number, multiplier = 1): number => Math.round(filters[idx] * multiplier);
