...
import { LocationPhase } from "@common/player";
        ...
        this.overwolf.infoUpdates$
            ...
            .subscribe((coord) => {
                const newCoordinates: MapCoordinates = {
                    x: parseInt((coord?.x as unknown) as string),
                    y: parseInt((coord?.y as unknown) as string),
                    z: parseInt((coord?.z as unknown) as string),
                };
                if (isNaN(newCoordinates.x) || isNaN(newCoordinates.y) || isNaN(newCoordinates.z)) return;
    ...
    private setupLandingCoordinates(): void {
        this.match.state$
            .pipe(
                ...
                filter((inUse) => !this.landingCoordinates$.value && !!inUse.length)
            )
            .subscribe(() => {
